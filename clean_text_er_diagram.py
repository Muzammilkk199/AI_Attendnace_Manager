#!/usr/bin/env python3
"""
Clean Text ER Diagram Generator for AI Attendance Manager
Creates a well-spaced, professional text-based ER diagram
"""

import os
import sys

class CleanTextERDiagramGenerator:
    def __init__(self):
        self.width = 140
        self.height = 50
        
    def create_clean_box(self, title, fields, width=35, height=18):
        """Create a clean text box for an entity"""
        lines = []
    ...
    def generate_clean_diagram(self):
        """Generate the complete clean text-based ER diagram"""
        diagram = []
        ...
        # Create entities
        student_box = self.create_clean_box("STUDENT (Django Model)", student_fields)
        attendance_box = self.create_clean_box("ATTENDANCE (Django Model)", attendance_fields)
        user_box = self.create_clean_box("USER (Django Model)", user_fields)
        mongodb_student_box = self.create_clean_box("MONGODB_STUDENTS (Collection)", mongodb_student_fields)
        mongodb_attendance_box = self.create_clean_box("MONGODB_ATTENDANCE (Collection)", mongodb_attendance_fields)
        
        # Arrange entities in a clean grid
    ...
    def save_clean_diagram(self, filename='ai_attendance_er_diagram_clean.txt'):
        """Save the clean diagram to a text file"""
        diagram = self.generate_clean_diagram()
        
        with open(filename, 'w', encoding='utf-8') as f:
            for line in diagram:
                f.write(line + '\n')
        
        print(f"✅ Clean text ER diagram saved as: {filename}")
    
    def print_clean_diagram(self):
        """Print the clean diagram to console"""
        diagram = self.generate_clean_diagram()
...
def main():
    """Main function to generate clean ER diagram"""
    try:
        print("🎨 Generating Clean AI Attendance Manager ER Diagram...")
        print("📐 Creating well-spaced, professional text-based diagram...")
        print("")
        
        # Create diagram generator
        generator = CleanTextERDiagramGenerator()
        
        # Print diagram to console
        generator.print_clean_diagram()
        
        # Save diagram to file
        generator.save_clean_diagram('ai_attendance_er_diagram_clean.txt')
        
        print("")
        print("🎉 Clean ER diagram generated successfully!")
        print("📁 Files created:")
        print("   - ai_attendance_er_diagram_clean.txt")
        print("")
        print("✨ Features:")
        print("   - Clean, professional spacing")
        print("   - Well-organized layout")
        print("   - Clear entity relationships")
        ...
        print(f"❌ Error generating clean ER diagram: {e}")
        import traceback
        traceback.print_exc()
